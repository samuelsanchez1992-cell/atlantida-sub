document.addEventListener('DOMContentLoaded', () => {
    ...
    // 12. Interactive Certifications Timeline
    ...
    const certData = {
        bautizo: {
            title: "Bautizo de Buceo (Discover Scuba Diving)",
            level: "Iniciación",
            desc: "Tu primera inmersión. Acompañado de un instructor del club, respirarás bajo el agua por primera vez en un entorno seguro, controlado y sin corrientes. Aprenderás las normas básicas de seguridad y a aletear.",
            depth: "6 metros",
            reqs: "Saber nadar y salud apta",
            outcome: "Primera toma de contacto con el mar"
        },
        owd: {
            title: "Open Water Diver (Buceador Autónomo)",
            level: "Básico / Oficial",
            desc: "El primer paso oficial. Aprenderás toda la teoría del buceo físico, fisiológico y de planificación de inmersiones. Practicarás ejercicios en aguas confinadas y realizarás 4 inmersiones en el mar con instructor.",
            depth: "18 metros",
            reqs: "Bautizo previo (recomendado) + Cert. Médico",
            outcome: "Licencia internacional de buceo autónomo"
        },
        aowd: {
            title: "Advanced Open Water Diver (Buceador Avanzado)",
            level: "Medio / Especialidades",
            desc: "Expande tus horizontes. Probarás inmersiones nocturnas, buceo profundo hasta 30 metros, navegación con brújula y orientación natural, mejora del control de flotabilidad y buceo en pecios.",
            depth: "30 metros",
            reqs: "Open Water Diver + 5 inmersiones registradas",
            outcome: "Habilitación para bucear en pecios y nocturno"
        },
        rescue: {
            title: "Rescue Diver (Buceador de Rescate)",
            level: "Avanzado / Seguridad",
            desc: "Aprende a anticipar problemas y reaccionar de forma experta. Practicarás rescate de buceadores en pánico, inconscientes, búsquedas submarinas, primeros auxilios y administración de oxígeno.",
            depth: "30 metros",
            reqs: "Advanced OWD + Curso de Primeros Auxilios (EFR)",
            outcome: "Ser el compañero de buceo más seguro y valorado"
        },
        pro: {
            title: "Divemaster & Instructor del Club",
            level: "Profesional / Liderazgo",
            desc: "El escalón profesional. Organiza, planifica y guía salidas oficiales del club. Ayuda a instructores en la enseñanza de alumnos, y ten la opción de formarte para ser instructor del propio club y certificar nuevos socios.",
            depth: "40 metros",
            reqs: "Rescue Diver + 60 inmersiones + Seguro de buceo",
            outcome: "Guía de inmersión y líder del club sin ánimo de lucro"
        }
    };
    ...
    // 13. Marine Conditions Zone Selector (Estado del Mar)
    const zoneBtns = document.querySelectorAll('.zone-btn');
    const seaTemp = document.getElementById('sea-temp');
    const seaTempSub = document.getElementById('sea-temp-sub');
    const seaWind = document.getElementById('sea-wind');
    const seaWindSub = document.getElementById('sea-wind-sub');
    const seaVisibility = document.getElementById('sea-visibility');
    const seaVisStatus = document.getElementById('sea-vis-status');
    const seaWaves = document.getElementById('sea-waves');
    const seaWavesStatus = document.getElementById('sea-waves-status');
    const marineStatusText = document.getElementById('marine-general-status');
    const pulseDot = document.querySelector('.pulse-dot');

    const zoneData = {
        sureste: {
            temp: "21°C",
            tempSub: "Recomendado: Húmedo 5mm",
            wind: "8 nudos (NE)",
            windSub: "Mar llana a rizada",
            vis: "25m",
            visStatus: "Excelente",
            visClass: "status-green",
            waves: "0.5m",
            wavesStatus: "Favorable",
            wavesClass: "status-green",
            status: "Condiciones excelentes para inmersiones desde costa y formación en Radazul y Tabaiba hoy.",
            dotColor: "#25d366"
        },
        sur: {
            temp: "22°C",
            ...
            wavesStatus: "Favorable",
            wavesClass: "status-green",
            status: "Mar de fondo leve en Las Galletas. Se recomiendan inmersiones desde barco para mejor comodidad.",
            dotColor: "#25d366"
        },
        norte: {
            temp: "19°C",
            tempSub: "Recomendado: Semiseco o 7mm",
            wind: "18 nudos (NW)",
            windSub: "Mar marejadilla a marejada",
            vis: "12m",
            visStatus: "Limitada",
            visClass: "status-yellow",
            waves: "1.6m",
            wavesStatus: "Precaución",
            wavesClass: "status-yellow",
            status: "Oleaje del norte fuerte hoy en Garachico. Extremar precaución en el acceso por costa o aplazar.",
            dotColor: "#eab308"
        }
    };
    ...
    if (zoneBtns.length > 0) {
        zoneBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if (data) {
                    seaTemp.textContent = data.temp;
                    seaTempSub.textContent = data.tempSub;
                    seaWind.textContent = data.wind;
                    seaWindSub.textContent = data.windSub;
                    seaVisibility.textContent = data.vis;
                    
                    // Update Visibility Status
                    seaVisStatus.textContent = data.visStatus;
                    seaVisStatus.className = `card-status ${data.visClass}`;

                    // Update Waves Status
                    seaWaves.textContent = data.waves;
                    seaWavesStatus.textContent = data.wavesStatus;
                    seaWavesStatus.className = `card-status ${data.wavesClass}`;

                    // Update General Status Text
                    marineStatusText.textContent = data.status;

                    // Update status pulse dot color
                    pulseDot.style.background = data.dotColor;
                    pulseDot.style.boxShadow = `0 0 0 0 ${data.dotColor}`;
                }
            });
        });
    }
